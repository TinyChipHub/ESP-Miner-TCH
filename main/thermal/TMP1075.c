...
static i2c_master_dev_handle_t tmp1075_alt_dev_handle;
...
esp_err_t TMP1075_init(void) {
    return i2c_bitaxe_add_device(TMP1075_I2CADDR_DEFAULT, &tmp1075_dev_handle, TAG) |
        i2c_bitaxe_add_device(TMP1075_I2CADDR_ALT, &tmp1075_alt_dev_handle, TAG);
}
...
uint8_t TMP1075_read_temperature(int device_index)
{
    ...
    if (device_index == 0) {
        ...
        //ESP_LOGI(TAG, "Reading temperature register");
        ESP_ERROR_CHECK(i2c_bitaxe_register_read(tmp1075_dev_handle, TMP1075_TEMP_REG, data, 2));
    }
    else {
        // read the alternative temperature register
        ESP_ERROR_CHECK(i2c_bitaxe_register_read(tmp1075_alt_dev_handle, TMP1075_TEMP_REG, data, 2));
    }
